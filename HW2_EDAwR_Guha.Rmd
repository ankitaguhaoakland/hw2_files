...
There seems to have one NA values for the variable Income, IncomeErr and ChildPoverty for the dataframe county_df.  
# Summary of county_df()
```{r}
summary(county_df)
```

# Summary of county_MI()
```{r}
summary(county_MI)
```
...
county_MI dataframe seems to be a sub dataset frame of county_df, without any NA values. 
...
# BoxPlot 
There needs to be some value corresposnding to y aesthetic, so used 1. 
```{r}
...
Summarising the findings in numerical order as in the graph below:
...
The datasets might have values that vary from each other on a large scale as seen in the scatterplots between Total Population and the Income as shown above. It is very hard to get a clarity on the data points whereby the datasets are overlapping on one another making it hard to comprehend any relationship between these two variables. This property of variability between two variables of data is due to the uneven variation of values with the mean value in the datasets often known as heteroscedastic variance. Using a log scale or, equivalently, using the log transform 'fixes' the uneven variability and gives a scale that is unbounded on both ends.

Also, for time-series data, comes from the ability of a log scale to make fractional changes equivalent. With log scale a constant percentage change is seen as a constant vertical distance so a constant growth rate is seen as a straight line. That is often a substantial advantage.
...
# v) Women Population Distribution in US Across Regions
```{r}
